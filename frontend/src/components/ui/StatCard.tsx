  ...
  ({ className, title, value, description, icon: Icon, trend, colorVariant = 'default', ...props }, ref) => {
    return (
      <div 
        ref={ref}
        className={cn(
          'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm transition-all hover:shadow-md hover:border-slate-300 dark:hover:border-slate-700 min-w-[160px]',
      ...
      >
        <div className="flex justify-between items-start mb-3">
          {Icon && (
            <div className={cn('p-2 rounded-lg', colorVariants[colorVariant])}>
        ...
        <div>
          <p className="text-sm text-slate-500 dark:text-slate-400 font-semibold mb-1 truncate">{title}</p>
          <p className={cn(
            "text-2xl font-bold truncate leading-tight dark:text-white",
        ...
        {trend?.label && (
          <p className="text-xs text-slate-400 mt-2 truncate">{trend.label}</p>
        )}
        {description && (
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-3 font-medium leading-relaxed border-t border-slate-100 dark:border-slate-800 pt-2">
             {description}
          </p>
        )}
      </div>
